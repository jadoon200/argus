"""Keyless RSS 2.0 / Atom ingester for world-news and agency feeds.

Feeds are untrusted input: parsing goes through defusedxml, which rejects
entity-expansion and external-entity (XXE) constructs outright. One broken or
blocked feed is logged and skipped, never fatal to the run.
"""
...
# Many publishers reject the default httpx User-Agent and 403 silently; a normal
# browser UA is required to read them.
...
def _clean(text: str | None) -> str | None:
    if not text:
        return None
    # Unescape first so feeds that *escape* their HTML (&lt;b&gt;) have the tags
    # revealed, then strip all tags, then collapse whitespace.
...
def _doc_id(source: str, link: str | None, fallback: str) -> str:
    digest = hashlib.sha256((link or fallback).encode()).hexdigest()[:24]
    return f"{source}:{digest}"


def _parse_rss_items(root: ElementTree.Element, source: str) -> list[Document]:
    docs = []
    for item in root.findall("./channel/item"):
        ...
        docs.append(
            Document(
                doc_id=_doc_id(source, link, item.findtext("guid") or title),
                source=source,
                title=title,
                summary=_clean(item.findtext("description")),
                url=link,
                published=_parse_rfc822(item.findtext("pubDate")),
                tags=[c.text for c in item.findall("category") if c.text] or None,
                raw={"author": author} if author else None,
            )
        )
    return docs
...
def _atom_link(entry: ElementTree.Element) -> str | None:
    """The entry's human-readable article URL (prefer rel=alternate / no rel)."""
...
def _parse_atom_entries(root: ElementTree.Element, source: str) -> list[Document]:
    docs = []
    for entry in root.findall(f"{ATOM_NS}entry"):
        ...
        docs.append(
            Document(
                doc_id=_doc_id(source, link, entry.findtext(f"{ATOM_NS}id") or title),
                source=source,
                title=title,
...
def parse_feed(xml_text: str, source: str = "rss") -> list[Document]:
    """Parse an RSS 2.0 or Atom feed; `source` is the provenance label per document."""
...
def fetch_rss_documents(feeds: Mapping[str, str] | None = None) -> list[Document]:
    """Fetch every configured feed; each document is tagged with its feed's label.

    `feeds` maps provenance label -> URL (defaults to settings.rss_feeds).
    """
    ...
    with httpx.Client(timeout=settings.http_timeout_seconds, headers=headers) as client:
        for source, url in (feeds if feeds is not None else settings.rss_feeds).items():
            try:
                docs.extend(parse_feed(_fetch_feed(client, url), source=source))
